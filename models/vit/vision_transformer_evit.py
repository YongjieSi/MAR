...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        # torch.Size([25, 12, 230, 64])
        attn = (q @ k.transpose(-2, -1)) * self.scale # torch.Size([25, 12, 230, 230])
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x) # torch.Size([25, 230, 768])
        x = self.proj_drop(x)
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            cls_attn = attn[:, :, 0, 1:]  # [B, H, N-1] torch.Size([25, 12, 229])
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,config=None,keep_rate=0.,fuse_token=False):
        super().__init__()
        self.config = config
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = self.proj(x).flatten(2).transpose(1, 2) #x:torch.Size([25, 1, 128, 201])
        return x # torch.Size([25, 228, 768]), 228=Floor((128-16)/10+1)*Floor((201-16)/10+1)
